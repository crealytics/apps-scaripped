...
/**
 * Fetches account-level callouts. Supports filtering and sorting.
 * <p>Typical usage:</p>
 * <pre class="prettyprint">
 *  var accountCalloutSelector = AdWordsApp.currentAccount().extensions()
 *      .callouts()
 *      .withCondition("Impressions &gt; 100")
 *      .forDateRange("LAST_MONTH")
 *      .orderBy("Clicks DESC");
 *  var accountCalloutIterator = accountCalloutSelector.get();
 *  while (accountCalloutIterator.hasNext()) {
 *    var accountCallout = accountCalloutIterator.next();
 *  }</pre>
 */
trait AccountCalloutSelector extends Selector[AccountCalloutSelector] {
  /**
   ...
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): AccountCalloutSelector = js.native
  /** Fetches the requested account-level callouts and returns an iterator. */
  def get(): AccountCalloutIterator = js.native
  /**
   ...
   * </ul>
   * <p>See <a href="adwordsapp_accountcalloutselector.html#withCondition_1">AccountCalloutSelector.withCondition(String)</a> for enumeration of columns that can be used.</p>
   ...
   * <p>If a stats column is used in the ordering, date range must be specified via <a href="adwordsapp_accountcalloutselector.html#forDateRange_1">AccountCalloutSelector.forDateRange(String)</a> or <a href="adwordsapp_accountcalloutselector.html#forDateRange_2">AccountCalloutSelector.forDateRange(Object, Object)</a>.</p>
   * <p><code>LabelNames</code> column cannot be used for ordering.</p>
   */
  def orderBy(orderBy: String): AccountCalloutSelector = js.native
  /**
   ...
   *  </tbody>
   * </table>
   * <p>If a stats column is used in the condition, date range must be specified via <a href="adwordsapp_accountcalloutselector.html#forDateRange_1">AccountCalloutSelector.forDateRange(String)</a> or <a href="adwordsapp_accountcalloutselector.html#forDateRange_2">AccountCalloutSelector.forDateRange(Object, Object)</a>. </p>
   */
  override def withCondition(condition: String): AccountCalloutSelector = js.native
  /**
   * Restricts this selector to return only account-level callouts with the given account-level callout IDs.
   * <pre class="prettyprint">
   *  var accountCalloutIds = [12345, 23456, 34567];
   *  selector = selector.withIds(accountCalloutIds);</pre>
   ...
   * <pre class="prettyprint">
   *  AdWordsApp.currentAccount().extensions().callouts()
   *     .withIds([12345, 23456, 34567])
   *     .withIds([34567, 45678, 56789]);</pre>
   * will only get the account-level callout with ID
   * <code>34567</code>
   * , since it would be the only account-level callout that satisfies both ID conditions.
   * <p></p>
   ...
   */
  def withIds(ids: js.Array[Long]): AccountCalloutSelector = js.native
}
