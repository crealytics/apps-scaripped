...
trait AudienceBidding {
  ...
  def getCpc: Double = ???
  /**  Returns the max CPM bid for this audience. */
  def getCpm: Double = ???
  /**
  ...
  /**
   *  Sets the max CPM bid for this audience. 
   * <p>Please note that although this method will effectively set the max CPM bid for this audience, the change may have no effect on actual bidding if this audience has a <a href="adwordsapp_biddingstrategy.html">BiddingStrategy</a> which does not involve max CPM bids.</p>
   *  
   * <p>Returns nothing.</p>
   *  
   */
