...
trait ExcludedTopicSelector {
   ...
   */
  def orderBy(orderBy: String): ExcludedTopicSelector = ???
  /**
   ...
   * </table>
   */
  def withCondition(condition: String): ExcludedTopicSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): ExcludedTopicSelector = ???
}