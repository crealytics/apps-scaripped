...
trait ExcludedPlacementSelector {
  ...
  def get: ExcludedPlacementIterator = ???
  /**
   ...
   */
  def orderBy(orderBy: String): ExcludedPlacementSelector = ???
  /**
   ...
   * </table>
   */
  def withCondition(condition: String): ExcludedPlacementSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): ExcludedPlacementSelector = ???
}