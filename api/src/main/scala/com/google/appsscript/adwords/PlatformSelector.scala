...
trait PlatformSelector {
   ...
   */
  def forDateRange(dateRange: String): PlatformSelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): PlatformSelector = ???
  /**  Fetches the requested platforms and returns an iterator. */
  def get: PlatformIterator = ???
  /**  Restricts this selector to only select mobile targets. */
  def mobile: PlatformSelector = ???
  /**
   ...
   */
  def orderBy(orderBy: String): PlatformSelector = ???
  /**  Restricts this selector to only select tablet targets. */
  def tablet: PlatformSelector = ???
  /**
   ...
   */
  def withCondition(condition: String): PlatformSelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Seq[Long]]): PlatformSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): PlatformSelector = ???
}