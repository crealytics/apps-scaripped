...
trait ReviewBuilder {
   ...
   * </ul>
   */
  def withEndDate(date: AnyRef): ReviewBuilder = ???
  /**
   ...
   * .
   */
  def withExactlyQuoted(isExactlyQuoted: Boolean): ReviewBuilder = ???
  /**
   *  Sets the review's device preference to mobile or clears it. This field is optional and defaults to 
   * <code>false</code>
   * .
   */
  def withMobilePreferred(isMobilePreferred: Boolean): ReviewBuilder = ???
  /**
   ...
   */
  def withSchedules(schedules: Seq[ExtensionSchedule]): ReviewBuilder = ???
  ...
  def withSourceName(sourceName: String): ReviewBuilder = ???
  /**  Sets the source URL of the new review to the specified value. This field is required. */
  def withSourceUrl(sourceUrl: String): ReviewBuilder = ???
  /**
   ...
   * </ul>
   */
  def withStartDate(date: AnyRef): ReviewBuilder = ???
  ...
  def withText(text: String): ReviewBuilder = ???
}