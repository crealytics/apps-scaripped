...
trait CalloutBuilder {
   ...
   * </ul>
   */
  def withEndDate(date: AnyRef): CalloutBuilder = ???
  /**
   *  Sets the callout's device preference to mobile or clears it. This field is optional and defaults to 
   * <code>false</code>
   * .
   */
  def withMobilePreferred(isMobilePreferred: Boolean): CalloutBuilder = ???
  /**
   ...
   */
  def withSchedules(schedules: Seq[ExtensionSchedule]): CalloutBuilder = ???
  /**
   ...
   * </ul>
   */
  def withStartDate(date: AnyRef): CalloutBuilder = ???
  /**  Sets the text of the new callout to the specified value. This field is required. */
  def withText(text: String): CalloutBuilder = ???
}