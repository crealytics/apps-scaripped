...
trait BiddingStrategySelector {
   ...
   */
  def forDateRange(dateRange: String): BiddingStrategySelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): BiddingStrategySelector = ???
  /**  Fetches the requested bidding strategies and returns an iterator. */
  def get: BiddingStrategyIterator = ???
  /**
   ...
   * </ul>
   *  
   * <p>See <a href="adwordsapp_biddingstrategyselector.html#withCondition_1">BiddingStrategySelector.withCondition(String)</a> for enumeration of columns that can be used.</p>
   *  
   ...
   */
  def withCondition(condition: String): BiddingStrategySelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Long]): BiddingStrategySelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): BiddingStrategySelector = ???
}