...
trait ExcludedAudienceSelector {
  ...
  def get: ExcludedAudienceIterator = ???
  /**
   ...
   */
  def orderBy(orderBy: String): ExcludedAudienceSelector = ???
  /**
   ...
   * </table>
   */
  def withCondition(condition: String): ExcludedAudienceSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): ExcludedAudienceSelector = ???
}