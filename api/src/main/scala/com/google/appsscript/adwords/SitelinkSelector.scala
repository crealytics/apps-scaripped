...
trait SitelinkSelector {
   ...
   */
  def forDateRange(dateRange: String): SitelinkSelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): SitelinkSelector = ???
  /**  Fetches the requested sitelinks and returns an iterator. */
  def get: SitelinkIterator = ???
  /**
   ...
   */
  def orderBy(orderBy: String): SitelinkSelector = ???
  /**
   ...
   */
  def withCondition(condition: String): SitelinkSelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Long]): SitelinkSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): SitelinkSelector = ???
}