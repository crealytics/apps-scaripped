...
trait PhoneNumberBuilder {
   ...
   */
  def build: PhoneNumberOperation = ???
  /**
   ...
   */
  def withCallOnly(callOnly: Boolean): PhoneNumberBuilder = ???
  /**  Sets the two character country code of the advertiser's phone number. This field is required. */
  def withCountry(country: String): PhoneNumberBuilder = ???
  /**
   ...
   */
  def withSchedules(schedules: Seq[ExtensionSchedule]): PhoneNumberBuilder = ???
  /**
   ...
   * </ul>
   */
  def withStartDate(date: AnyRef): PhoneNumberBuilder = ???
}