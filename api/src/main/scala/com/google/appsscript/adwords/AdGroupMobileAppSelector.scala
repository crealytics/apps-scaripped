...
trait AdGroupMobileAppSelector {
   ...
   */
  def forDateRange(dateRange: String): AdGroupMobileAppSelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): AdGroupMobileAppSelector = ???
  /**  Fetches the requested ad group-level mobile apps and returns an iterator. */
  def get: AdGroupMobileAppIterator = ???
  /**
   ...
   */
  def orderBy(orderBy: String): AdGroupMobileAppSelector = ???
  /**
   ...
   */
  def withCondition(condition: String): AdGroupMobileAppSelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Long]): AdGroupMobileAppSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): AdGroupMobileAppSelector = ???
}