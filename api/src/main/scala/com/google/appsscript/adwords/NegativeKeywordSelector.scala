...
trait NegativeKeywordSelector {
  ...
  def get: NegativeKeywordIterator = ???
  /**
   ...
   */
  def orderBy(orderBy: String): NegativeKeywordSelector = ???
  /**
   ...
   * </table>
   */
  def withCondition(condition: String): NegativeKeywordSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): NegativeKeywordSelector = ???
}