...
trait CalloutSelector {
   ...
   */
  def forDateRange(dateRange: String): CalloutSelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): CalloutSelector = ???
  /**  Fetches the requested callouts and returns an iterator. */
  def get: CalloutIterator = ???
  /**
   ...
   */
  def orderBy(orderBy: String): CalloutSelector = ???
  /**
   ...
   */
  def withCondition(condition: String): CalloutSelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Long]): CalloutSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): CalloutSelector = ???
}