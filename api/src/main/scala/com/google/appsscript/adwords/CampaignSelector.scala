...
trait CampaignSelector {
   ...
   */
  def forDateRange(dateRange: String): CampaignSelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): CampaignSelector = ???
  /**  Fetches the requested campaigns and returns an iterator. */
  def get: CampaignIterator = ???
  /**
   ...
   */
  def orderBy(orderBy: String): CampaignSelector = ???
  /**
   ...
   */
  def withCondition(condition: String): CampaignSelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Long]): CampaignSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): CampaignSelector = ???
}