...
trait AdCustomizerItemSelector {
   ...
   */
  def forDateRange(dateRange: String): AdCustomizerItemSelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): AdCustomizerItemSelector = ???
  /**  Fetches the requested ad customizer items and returns an iterator. */
  def get: AdCustomizerItemIterator = ???
  /**
   ...
   * </ul>
   *  
   * <p>See <a href="adwordsapp_adcustomizeritemselector.html#withCondition_1">AdCustomizerItemSelector.withCondition(String)</a> for enumeration of columns that can be used.</p>
   *  
   ...
   */
  def withCondition(condition: String): AdCustomizerItemSelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Long]): AdCustomizerItemSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): AdCustomizerItemSelector = ???
}