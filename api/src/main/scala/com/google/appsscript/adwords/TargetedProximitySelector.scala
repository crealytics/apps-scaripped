...
trait TargetedProximitySelector {
   ...
   */
  def forDateRange(dateRange: String): TargetedProximitySelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): TargetedProximitySelector = ???
  /**  Fetches the requested targeted proximities and returns an iterator. */
  def get: TargetedProximityIterator = ???
  /**
   ...
   * </ul>
   *  
   * <p>See <a href="adwordsapp_targetedproximityselector.html#withCondition_1">TargetedProximitySelector.withCondition(String)</a> for enumeration of columns that can be used.</p>
   *  
   ...
   */
  def withCondition(condition: String): TargetedProximitySelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Seq[Long]]): TargetedProximitySelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): TargetedProximitySelector = ???
}