...
trait AdGroupReviewSelector {
   ...
   */
  def forDateRange(dateRange: String): AdGroupReviewSelector = ???
  /**
   ...
   * <p>Date range must be specified if the selector has conditions or ordering for a stat field. Note that only the last date range specified for the selector will take effect.</p>
   */
  def forDateRange(dateFrom: AnyRef, dateTo: AnyRef): AdGroupReviewSelector = ???
  /**  Fetches the requested ad group-level reviews and returns an iterator. */
  def get: AdGroupReviewIterator = ???
  /**
   ...
   */
  def orderBy(orderBy: String): AdGroupReviewSelector = ???
  /**
   ...
   */
  def withCondition(condition: String): AdGroupReviewSelector = ???
  /**
   ...
   * <p></p>
   */
  def withIds(ids: Seq[Long]): AdGroupReviewSelector = ???
  /**
   *  Specifies limit for the selector to use. For instance, 
   * <code>withLimit(50)</code>
   *  returns only the first 50 entities.
   */
  def withLimit(limit: Int): AdGroupReviewSelector = ???
}