...
trait EmbeddedPieChartBuilder {
  ...
  def setColors(cssValues: Seq[String]): EmbeddedPieChartBuilder = ???
  ...
  def setTitleTextStyle(textStyle: TextStyle): EmbeddedPieChartBuilder = ???
}